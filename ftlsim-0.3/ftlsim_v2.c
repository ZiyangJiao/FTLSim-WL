...
void do_ftl_run(struct ftl *ftl, struct getaddr *addrs, int count)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        /***WL***/
        if (ftl->wl_threshold != 0 && ftl->ext_writes > 0 && ftl->ext_writes % ftl->wl_threshold == 0) {
            if (ftl->wl_activated == 1) { // Greedy
                int random = rand() % (ftl->T*ftl->Np);
//                printf("random = %d\n", random);
                while (ftl->map[random].block == NULL) {
                  random = rand() % (ftl->T*ftl->Np);
                }
                struct segment *target = ftl->map[random].block;
                for (j = 0; j < target->Np; j++) {
                  if (target->lba[j] != -1) {
                    ftl->wl_counts++;
                    pool = ftl->get_input_pool(ftl, target->lba[j]);
                    pool->write(ftl, pool, target->lba[j]);
                    check_new_segment(ftl, pool);
                  }
                }
            } else { // LRU
              // no need to perform WL because of the nature of LRU
            }
        }
        /***WL***/
    }
...
//        int random = ftl->ext_writes % ftl->Np;
//        if (pool->tail != NULL){
//            struct segment *target = pool->frontier;
//            while (random != 0 && target->next->next != NULL) {
//                target = target->next;
//                random--;
//            }
//            if (target->prev->next != NULL && target->next->prev!= NULL) {
//                target->prev->next = target->next;
//                target->next->prev = target->prev;
//
//                target->prev = pool->frontier;
//                target->next = pool->frontier->next;
//                pool->frontier->next = target;
