...
void do_ftl_build_heap(struct ftl *ftl){
    printf("re_ec_min: %d,%d\n", ftl->ec_min->size,ftl->ec_min->capacity);
    if (ftl->ec_min->size != 0) {
        ECMinHeap* tmp = init_ecminheap(ftl->T + ftl->Np);
        int i = 0;
        for (i; i < ftl->ec_min->size; i++) {
            insert_ecminheap(tmp, ftl->ec_min->arr[i]);
        }
...
//            gc = 1;
        ...
        ///***WL: Start***///
        if (ftl->wl_activated == 1 && mapped_out == 0) {
            ecmin_addone_check(ftl->ec_min, b->ecmin_index);

            //pwl formula
            int erase_avg = ftl->erase_counts / ftl->T;
            int init_threshold = ftl->endurance / 4 * 3;
            int pwl_threshold = erase_avg * (ftl->endurance - init_threshold) / ftl->endurance + init_threshold;

            struct segment *coldest = get_ecmin(ftl->ec_min);
//                printf("%d\n",coldest->erase_counts);
            if (coldest != NULL && coldest->n_valid > 0 && b->erase_counts > pwl_threshold) {
                 // record WL overhead
                ftl->wl_counts++;
