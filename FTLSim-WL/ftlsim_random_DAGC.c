...
static struct segment *list_popold(struct segment *list) // for cost-benefit
{
...
int do_ftl_write(struct ftl *ftl, int addrs) {
    ...
    if (ftl->nfree == 0 && ftl->shrinkable == 0) {
    	printf("nfree equals 0 before GC\n");
        return 1;
    }
...
//        ftl->TBW++;
//    }
    /***DAGC threshold: Start***/
//    float ECP = (ftl->T - ftl->U) / ftl->T;
//    float FPP = ftl->nfree / ftl->T;
//    float VPP = pool->pages_valid / (ftl->T * pool->Np);
//    float IPP = pool->pages_invalid / (ftl->T * pool->Np);
//    float IGCT = 0.2 * ECP + 0.1 * (1-VPP) + 0.15 * IPP;
//    float NGCT = 0.05 * ECP + 0.1 * (1-VPP);
//    if (FPP < IGCT) {
//        ftl->minfree = 100;
//    } else {
//    	if (ftl->nfree < 500) {
//    	    ftl->minfree = ftl->nfree + 1;
//    	} else {
//    	    ftl->minfree = 100;
//    	}
//    }
...
//            if (ftl->wl_activated == 1) {
        ...
        b->gc_counts++;
        // for DAGC
        if (b->erase_counts > ftl->max_erase) {
          ftl->max_erase = b->erase_counts;
        }
...
static struct segment *list_popDAGC(struct pool *pool) // for DAGC
{
  struct segment *tmp = NULL;
  struct segment *b = NULL;
//  float lamda = (float)pool->ftl->nfree / pool->ftl->T;
//  float lamda = 0.6; // smaller value, closer to greedy
//  float score = -1;
  int random = greedy_tail_n_valid(pool);
  if (rand() % 3 == 1) {
      random = rand() % (pool->ftl->Np + 1);
      while (list_empty(&pool->bins[random])) {
          random = rand() % (pool->ftl->Np + 1);
      }
  }
  for (tmp = (&pool->bins[random])->next; tmp != &pool->bins[random]; tmp = tmp->next) {
      if (b == NULL || tmp->erase_counts < b->erase_counts) {
          b = tmp;
      }
  }
  ...
  int min = greedy_tail_n_valid(pool);
  pool->ftl->wl_writes += random - min;
  return b;
}
...
//    }
    ...
    // b->in_pool = 0;
    /***Cost-Benefit***/ 
    // struct segment *b = list_popold(&pool->bins[i]);
    // b->in_pool = 0;
    /***DAGC***/
    struct segment *b = list_popDAGC(pool);
    if (b == NULL) {
      printf("list_popDAGC return NULL\n");
      return NULL;
    }
//    if (b->erase_counts > 4) {
//        printf("list_popDAGC return a block with %d valid pages\n", b->n_valid);
//    }
    b->in_pool = 0;
